...
const PRODUCT_SELECTOR_TIMEOUT_MS = 20_000;
...
const FLIPKART_SELECTORS = {
  ...
  rating: "div.XQDdHH",
  outOfStock:
    'div[class*="out-of-stock"], div[class*="OutOfStock"], div[class*="outOfStock"]',
} as const;
  ...
  try {
    ...
    try {
      ...
      throw new ScrapeError(
        `Page failed to load: navigation timed out or was aborted for ${url}`,
      );
    }
    ...
    if (await isBlockedOrErrorPage(page)) {
      throw new ScrapeError(
        "Page failed to load: Flipkart returned an error or blocked the request",
      );
    }
...
async function waitForProductContent(page: Page): Promise<void> {
  const productLocator = page.locator(
    `${FLIPKART_SELECTORS.name}, ${FLIPKART_SELECTORS.price}, script[type="application/ld+json"]`,
  );
  ...
  try {
    ...
    throw new ScrapeError(
      "Page failed to load: product content did not appear in time",
    );
  }
}

async function isBlockedOrErrorPage(page: Page): Promise<boolean> {
  const bodyText = await page.locator("body").innerText();
  const title = await page.title();

  return (
    /\bE002\b/i.test(bodyText) ||
...
async function extractFromJsonLd(page: Page): Promise<PartialProduct> {
...
function mapSchemaProduct(record: Record<string, unknown>): PartialProduct {
  const offers = record.offers as Record<string, unknown> | undefined;
  const aggregateRating = record.aggregateRating as
  ...
  const availabilityRaw =
    offers?.availability ??
    (Array.isArray(offers)
      ? (offers[0] as Record<string, unknown>)?.availability
      : undefined);
...
function formatSchemaAvailability(value: unknown): string | undefined {
  if (value == null) return undefined;

  const text = String(value);
  if (/InStock/i.test(text)) return "In stock";
  if (/OutOfStock/i.test(text)) return "Out of stock";
  if (/PreOrder/i.test(text)) return "Pre-order";
  if (/LimitedAvailability/i.test(text)) return "Limited availability";

  return text.replace(/^https?:\/\/schema\.org\//i, "").trim() || undefined;
}

async function extractFromDom(page: Page): Promise<ProductData> {
...
async function getFieldText(page: Page, selector: string): Promise<string> {
...
async function getAvailability(page: Page): Promise<string> {
